...
void setPosition(int count, int* p_HPPotion, int* p_MPPotion)
{
	*p_HPPotion = count;
	*p_MPPotion = count;
	...
	cout << "HP 포션" << *p_HPPotion << "개, MP 포션" << *p_MPPotion << "개가 기본지급 되었습니다." << endl;
	cout << "=====================" << endl;
	cout << "캐릭터 강화" << endl;
	...
	while (!isgameStart)
	{
		...
		case 1:
			...
			if (*p_HPPotion < 0)
			{
				...
				*p_HPPotion = 0; // 포션 개수를 0으로 유지
				break;
			}
			cout << "HP가 20 증가했습니다. (HP 포션차감	: 남은포션 " << *p_HPPotion << ")" << endl;
			break;
		case 2:
			if (*p_MPPotion > 0)
			{
			...
			if (*p_MPPotion < 0)
			{
				...
				*p_MPPotion = 0; // 포션 개수를 0으로 유지
				break;
			}
			cout << "MP가 20 증가했습니다. (MP 포션차감	: 남은포션 " << *p_MPPotion << ")" << endl;
			break;
		case 3:
			stat[2] *= 2;
